...
export const NoteProvider = ({ children }) => {
  ...
  const getNotes = async () => {
    ...
    try {
      ...
      setNotes(response.data.notes); // assuming backend returns { notes: [...] }
    } catch (error) {
  ...
  const createNote = async (note) => {
    ...
    setNotes((prev) => [res.data.note, ...prev]); // ✅ functional update
  };
  ...
  const deleteNote = async (id) => {
    ...
    setNotes((prev) => prev.filter((note) => note._id !== id)); // ✅ functional update
  };
